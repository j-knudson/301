...
void arrayMaker(int A[], int B[], int C[], int D[], pQueue& Q, int size, int seed, bool print);	//fills arrays with random values and if selected prints those values to terminal
...
void print_pQueue(pQueue q, int size);
void heapSort(int A[], int size, int& count);
void reheapDown(int A[], int root, int last, int& count);
...
int main()
{
	...
	int* array4 = new int[userArrSize];			//fourth array for Heap Sort
	pQueue queue1;

	arrayMaker(array1, array2, array3, array4, queue1, userArrSize, userSeed, displayValues); //call function to make three identical arrays of random values 

	int comparisons = insertionSort(array1, userArrSize);							//create a variable to hold the efficiency number for array sorting and try Insertion Sort

	std::cout << std::endl << "Insertion Sort count = " << comparisons << std::endl;	//Display Insertion Sort Results
	//printArray(array1, userArrSize); std::cout << std::endl; 
	comparisons = 0;													//variable for merge sort comparisons 
	...
	//printArray(array3, userArrSize - 1);
	std::cout << "\n"<< "Quick Sort count = " << comparisons << std::endl; //display results

	//printArray(array3, userArrSize);

	//print_pQueue(queue1, userArrSize);
	std::cout << '\n';
	//queue1.exchange(0, 1);
	queue1.enqueue(42);
	//queue1.exchange(0, 9);
	//print_pQueue(queue1, userArrSize + 1);
	std::cout << "\n ----------\n";

	comparisons = 0; 
	//printArray(array4, userArrSize);
	std::cout << '\n';
	heapSort(array4, userArrSize, comparisons);
	//printArray(array4, userArrSize);
	std::cout << '\n' << "Heap Sort count = " << comparisons;
...
void arrayMaker(int A[], int B[], int C[], int D[], pQueue& Q, int size, int seed, bool print)
{
	srand(seed);							//for use with rand function
	for (int i = 0; i < size; ++i)
	{
		A[i] = rand() % 99;				//fill array with values having a max of 4 digits
		B[i] = A[i];
		C[i] = A[i];
		D[i] = A[i];
		Q.enqueue(A[i]);
...
void reheapDown(int A[], int root, int last, int& count)
{
	...
	parent = root; 
	while (2*parent < last && !done)
	{
		++count;
		maxChild = 2 * parent + 1; 
		if (maxChild < last && A[maxChild + 1]>A[maxChild])  //check if right child larger
			++maxChild;
